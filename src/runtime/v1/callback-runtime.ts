...
export async function acceptCallbackEnvelope(body: string, configurationVersion: string, repository: string, inbox: Pick<PostgresWebhookInbox, "accept">): Promise<void> {
...
export async function runCallbackRuntime(environment: NodeJS.ProcessEnv = process.env): Promise<void> {
  ...
  const dynamo = config.CALLBACK_RUNTIME_MODE === "ingress" ? new DynamoDBClient({ region: "us-east-1" }) : undefined;
  ...
  const worker = resolver ? new CallbackWorker(control, inbox, new PostgresSprintRunRepository(pool), resolver, inbox, { ownerId: callbackWorkerId(), configurationVersion: config.RUNTIME_CONFIGURATION_VERSION, maxBatch: 10, maxAttempts: 5, leaseMilliseconds: 120_000 }) : undefined;
  try {
    while (control.mayClaim) {
      if (sqs && dynamo) {
      ...
      await publishCallbackEvents(pool, dynamo, config.COORDINATION_TABLE_NAME);
      }
      if (worker) {
