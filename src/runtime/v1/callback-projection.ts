...
export async function publishCallbackEvents(pool: Pool, client: DynamoProjectionClient, tableName: string, mayWork?: () => Promise<boolean>): Promise<number> {
  ...
  for (const row of rows.rows) {
    if (mayWork && !await mayWork()) throw new Error("callback_projection_fenced");
