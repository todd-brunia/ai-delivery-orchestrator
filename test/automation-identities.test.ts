...
describe("automation-identities/v1", () => {
  ...
  it("requires unique roles, app IDs, installations, and secret containers", () => {
    const builder = { ...contract, role: "builder", appSlug: "todd-brunia-ai-delivery-builder", appId: "1002", installationId: "2002", permissionCeiling: ["metadata:read", "contents:write", "issues:write", "actions:write", "pull_requests:write", "checks:read"], allowedOperations: ["set_workflow_labels", "dispatch_allowlisted_workflow", "mark_exact_head_ready_for_review"], secretContainerName: contract.secretContainerName.replace("reviewer", "builder") };
    expect(() => AutomationIdentityContractSchema.parse({ ...builder, permissionCeiling: builder.permissionCeiling.filter((permission) => permission !== "checks:read") })).toThrow();
    expect(() => AutomationIdentityContractSchema.parse({ ...builder, permissionCeiling: builder.permissionCeiling.map((permission) => permission === "checks:read" ? "checks:write" : permission) })).toThrow();
