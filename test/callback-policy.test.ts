...
describe("callback decision policy", () => {
  ...
  it("uses canonical workflow evidence for build start and produces a stable semantic key", () => {
    ...
    expect(first).toMatchObject({ route: "workflow", disposition: "ready", events: ["build_started", "pull_request_opened", "checks_awaited"] });
  ...
  it("fails closed on identity, immutable binding, head, and required-check drift", () => {
    ...
    expect(decideCallback(hint, binding, { ...observed, requiredChecks: { lint: "success", tests: "pending" } }, "pr_open")).toMatchObject({ disposition: "ready" });
  });
