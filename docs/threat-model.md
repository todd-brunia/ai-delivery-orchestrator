...
Repository-configuration validation reasons are likewise closed to `missing`, `wrong_type`, `invalid_value`, and `unknown_reason`. The adapter derives the classification while the untrusted canonical response is still local, then throws only a strict, versioned data record with a closed internal field token and reason. Exact-shape parsing, rather than JavaScript prototype identity, carries that record through the compiled runtime. The diagnostic layer maps the token to its own field enum and never accepts the raw response, Zod issue, message, code, input, or allowed-value list; extra properties fail closed as `unexpected`. Reasons are diagnostic evidence only and grant no permission, schema, retry, deployment, or dispatch authority.
