...
An `invalid_input` failure for `repository_configuration` may include one closed `field`: `repository`, `repository_id`, `default_branch`, `visibility`, `archive`, `fingerprint`, `evidence`, or `unknown_field`. This is a translation of the first internal schema path segment, never the path or value itself. `unknown_field` means attribution ended safely; it is not permission to log validation details, inspect credentials, relax the schema, or retry a task.
