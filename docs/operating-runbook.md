...
Private DNS routes AWS Secrets Manager, ECR, and CloudWatch Logs traffic to the pilot interface endpoints even when the supervised task has a public IP. Their shared endpoint security group accepts TCP 443 from only the worker/migration and supervised task security groups through separate exact SG-to-SG rules. Never replace the supervised source with a CIDR, prefix list, public source, or full-VPC allowance. An endpoint initialization failure consumes that task attempt and grants no retry authority.
...
A nonzero supervised exit emits exactly one `supervised-runtime-diagnostic/v1` record. Its `stage` is a static authority boundary (`configuration`, `secret_access`, `canonical_read`, `database`, `model_analysis`, `policy`, `execution_gate`, or `unexpected`) and its `category` is an allowlisted provider/input code. The record never contains exception messages, stacks, response bodies, headers, URLs, prompts, artifacts, repository text, database values, or credentials. Use the stage and category only to select the next investigation boundary. A diagnostic is evidence of failure, not permission to inspect secret values, broaden logging, retry a task, deploy, or dispatch.

For `canonical_read` failures only, the record may also include one static `operation`: `default_branch_ref`, `workflow_at_ref`, `issue`, `marked_plan`, `repository_configuration`, `installation`, or `human_approval`. The operation identifies which bounded adapter method failed; it contains no provider field, schema path, or validation message. Do not infer permission to extract an App credential or bypass the named check. An absent operation remains fail closed and requires reviewed diagnostic coverage before another task authorization.

When one canonical adapter method calls another, the innermost failing allowlisted operation is authoritative. For example, repository-configuration validation reached through a default-branch lookup must report `repository_configuration`, while a failure in the ref request itself reports `default_branch_ref`. Treat an outer-operation label for a known nested failure as diagnostic instrumentation drift; do not change either canonical policy based on that label.

An `invalid_input` failure for `repository_configuration` may include one closed `field`: `repository`, `repository_id`, `default_branch`, `visibility`, `allow_squash_merge`, `archive`, `fingerprint`, `evidence`, or `unknown_field`. This is a translation of the first internal schema path segment, never the path or value itself. `unknown_field` means attribution ended safely; it is not permission to log validation details, inspect credentials, relax the schema, or retry a task.

The same field-attributed failure may include one closed `reason`: `missing`, `wrong_type`, `invalid_value`, or `unknown_reason`. The adapter derives it before discarding the untrusted response and passes only a strict, versioned data record to the diagnostic boundary; classification does not depend on JavaScript error-prototype identity. Raw schema issues, paths, messages, codes, inputs, and allowed-value lists remain prohibited. Use the reason only to identify the next read-only investigation. It does not authorize a permission change, schema relaxation, retry, deployment, or dispatch.

An `unexpected` failure for `repository_configuration` may instead include one closed `checkpoint`: `response_read`, `snapshot`, `schema_validation`, `failure_handoff`, or `unknown_checkpoint`. This locates only the static internal boundary that replaced an unknown exception. It never contains or implies the exception type, message, stack, provider response, schema issue, field, or value. Treat the checkpoint as evidence for a reviewed code investigation, not permission to broaden logging, retry, change networking or credentials, deploy, or dispatch.

A nested `secret_access` failure remains the narrower authority stage and must omit the canonical operation and checkpoint. A rejected supervised GitHub fetch is translated to the static `canonical_read / transport` category with its allowlisted operation; the original fetch exception is discarded. Neither result distinguishes DNS, routing, TLS, timeout, credential content, or provider text, and neither authorizes a retry or configuration change.

The supervised PostgreSQL client loads only the reviewed `certificates/aws-rds-us-east-1-rsa2048-g1.pem` asset copied into the immutable image. The same path is configured through `NODE_EXTRA_CA_CERTS`, and the client must retain `rejectUnauthorized=true`. Treat a missing file, filename mismatch, invalid PEM, or configuration-stage failure as an image integrity defect. Do not download a replacement, disable verification, broaden trust, or retry a task under the consumed authorization.
