...
const taskArn = z.string().regex(new RegExp(`^arn:aws:ecs:${region}:${account}:task/${cluster}/[a-f0-9]{32}$`));
const count = z.number().int().nonnegative();
const status = z.enum(["PROVISIONING", "PENDING", "ACTIVATING", "RUNNING", "DEACTIVATING", "STOPPING", "DEPROVISIONING", "STOPPED", "DELETED"]);
const task = z.object({
  taskArn, taskDefinitionArn: z.string().regex(new RegExp(`^arn:aws:ecs:${region}:${account}:task-definition/ai-delivery-orchestrator-pilot-(worker|supervised-dispatch):[0-9]+$`)),
...
export function collectCallbackProgress(taskArns = [], read = aws) {
  z.array(taskArn).max(3).parse(taskArns);
  ...
  if (taskArns.length) {
    ...
    tasks = z.object({ tasks: z.array(task), failures: z.array(z.unknown()).length(0) }).parse(normalized).tasks;
